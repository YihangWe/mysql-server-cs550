 ...
 */
 ...
 /** Grant a lock to waiting transactions using the bLDSF algorithm. This function
 scans the queue of locks in which in_lock resides (or resided) paying attention
 only to locks on heap_no-th bit. For each waiting lock which was blocked by in_lock->trx
 it checks if it can be granted now.
 
 For low priority transactions, which locks will be granted is calculated based on the
 maximum scheduling weight after factoring in the decay function, which approximates
 the likelihood of transactions having to wait for an exclusive lock.
 High priority transactions will automatically move to the front of the queue.
 ...
 @param[in]    heap_no   Heap number within the page on which the
 lock was (or still is) held */
 static void lock_rec_grant_by_heap_no_bldsf(lock_t *in_lock, ulint heap_no) {
   ut_ad(in_lock->is_record_lock());
   ut_ad(locksys::owns_page_shard(in_lock->rec_lock.page_id));
   ...
   RecID rec_id{in_lock, heap_no};
   Locks<LockDescriptorEx> low_priority{heap.get()};
   Locks<lock_t *> waiting{heap.get()};
   Locks<lock_t *> granted{heap.get()};
   ...
   in_lock->hash_table().find_on_record(rec_id, [&](lock_t *lock) {
     ...
     - granted = granted locks
     - waiting = waiting locks of high priority transactions
     - low_priority = waiting locks that have low priority will be sorted by schedule
       weight
     */
     if (!lock->is_waiting()) {
       /* Granted locks should be before waiting locks. */
     ...
     relative order between this load and other operations.
     */
     low_priority.push_back(
       LockDescriptorEx{trx->lock.schedule_weight.load(std::memory_order_relaxed), lock});
 
     return false;
   });
 
   if (waiting.empty() && low_priority.empty()) {
     /* Nothing to grant. */
     return;
   }
   /* We want high schedule weight to be in front, and break ties by position */
   std::stable_sort(low_priority.begin(), low_priority.end(),
                    [](const LockDescriptorEx &a, const LockDescriptorEx &b) {
                      return (a.first > b.first);
                    });
   // for (const auto &descriptor : low_priority) {
   //   waiting.push_back(descriptor.second);
   // }
 
   /* New granted locks will be added from this index. */
   const auto new_granted_index = granted.size();
   granted.reserve(granted.size() + waiting.size() + low_priority.size());
 
   /* Attempt to grant all high-priority locks
    */
   for (lock_t *wait_lock : waiting) {
     /* Check if the transactions in the waiting queue have
     to wait for locks granted above. If they don't have to
   ...
   /* Based on the bLDSF algorithm, the benefits of granting locks to the highest-performing
    * algorithm decay with each additional lock. Find the maximum value of the total
    * weight of locks to be granted, divided by the decay factor.
    * 
    * The decay function is set to be log_2(1+k). Theoretically any sublinear increasing
    * function would work.
   */
   float max = 0;
   for (std::size_t k = 1; LockDescriptorEx lock_desc : low_priority) {
   // for (std::size_t i = 0, e = low_priority.size(); i != e; ++i) {
     ut_ad(lock_desc.second->trx != in_trx);
     
     const lock_t *blocking_lock =
     lock_rec_has_to_wait_for_granted(lock_desc.second, granted, new_granted_index);
     if (blocking_lock == nullptr) {
       if ((max + lock_desc.first) / log2(1 + k) > max / log2(k)) {
         lock_grant(lock_desc.second);
       } else {
         break;
       }
       lock_rec_move_granted_to_front(lock_desc.second, rec_id);
 
       granted.push_back(lock_desc.second);
     } else {
       lock_update_wait_for_edge(lock_desc.second, blocking_lock);
     }
     ++k;
   }
 }
   ...
   if (in_lock->hash_table().find_on_page(
     ...
     for (ulint heap_no = 0; heap_no < lock_rec_get_n_bits(in_lock); ++heap_no) {
       if (lock_rec_get_nth_bit(in_lock, heap_no)) {
         #ifdef LOCK_BLDSF
         lock_rec_grant_by_heap_no_bldsf(in_lock, heap_no);
         #else
